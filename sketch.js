...
let resultMessage = "";
let resultTimer = 0;
let questionPosition = { x: 0, y: 0 }; // 題目數字的位置
let questions = [
  {
...
function setup() {
  ...
  // 初始化題目數字的位置
  questionPosition.x = random(100, width - 100);
  questionPosition.y = random(100, height - 100);
}
...
function draw() {
  ...
  if (showResult && millis() - resultTimer > 1000) {
    ...
    questionPosition.x = random(100, width - 100); // 更新題目數字的位置
    questionPosition.y = random(100, height - 100);
  }
}
...
function displayQuestion() {
  ...
  textSize(32);
  textAlign(CENTER, CENTER);
  text(currentQuestionIndex + 1, questionPosition.x, questionPosition.y);
